...
test('encaminha login, corpo e query, preserva Set-Cookie e desativa cache', async () => {
  ...
  const response = await worker.fetch(
    request('/api/v1/admin/auth/login?source=test', {
      method: 'POST',
      body: JSON.stringify({ email: 'admin@figo.test', password: 'test-only' }),
      headers: {
        'Content-Type': 'application/json',
        Cookie: 'another=secret; figo_admin=previous',
...
test('GET sem Origin envia a origem do backoffice para a allowlist do backend', async () => {
  mock.method(globalThis, 'fetch', async (req) => {
    ...
    return Response.json(
      { success: false, error: { code: 'ADMIN_SESSION_REQUIRED' } },
      { status: 401 }
    );
  });
  const response = await worker.fetch(
...
test('não segue redirects nem expõe erros de rede internos', async () => {
  const fetch = mock.method(
    ...
    () =>
      new Response(null, {
        status: 302,
        headers: { Location: 'https://another.test', 'Set-Cookie': 'secret=value' },
      })
  );
  ...
  fetch.mock.mockImplementation(() => {
    throw new Error('Internal credentials and addresses');
  });
...
test('rejeita destino inválido, HTTP remoto e recursão para o próprio Worker', async () => {
  ...
  for (const BACKEND_ORIGIN of [
    undefined,
    '',
    'not-url',
    origin,
    'http://backend.figo.test',
    'https://user:pass@backend.figo.test',
    'https://backend.figo.test/api/v1',
    'https://backend.figo.test/?target=evil',
    'https://backend.figo.test/#hash',
  ]) {
...
test('imagens preservam query e bytes, sem cookies e sem cache', async () => {
  ...
  const response = await worker.fetch(
    request('/uploads/products/photo.png?variant=thumb', {
  ...
  assert.equal(
    (await worker.fetch(request('/uploads/products/photo.png', { method: 'POST' }), env)).status,
    405
  );
});
...
test('CSV e logout preservam download e remoção da sessão', async () => {
  ...
  const cookie =
    'figo_admin=; Path=/api/v1/admin; Expires=Thu, 01 Jan 1970 00:00:00 GMT; HttpOnly; Secure; SameSite=Strict';
  fetch.mock.mockImplementation(() =>
    Response.json({ success: true }, { headers: { 'Set-Cookie': cookie } })
  );
