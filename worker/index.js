...
function backendOrigin(value, frontend) {
  ...
  if (
    ...
    backend.pathname !== '/' || backend.origin === frontend.origin
...
export default {
  async fetch(request, env) {
    ...
    if (admin && (
      ...
      return errorResponse(403, 'ADMIN_ORIGIN_FORBIDDEN', 'Pedido de administração não autorizado.');
    }
    ...
    try {
      ...
      return errorResponse(503, 'BACKEND_NOT_CONFIGURED', 'A ligação ao servidor não está configurada.');
    }
    ...
    if (admin) {
      ...
      const sessionCookie = (request.headers.get('Cookie') || '').split(';')
        .map(value => value.trim()).find(value => value.startsWith('figo_admin='));
    ...
    try {
      ...
      if (response.status >= 300 && response.status < 400 && response.status !== 304) {
        ...
        return errorResponse(502, 'BACKEND_REDIRECT', 'O servidor devolveu um redirecionamento inesperado.');
      }
      // Keeps Set-Cookie (including deletion), HttpOnly, Secure, SameSite and its path intact.
      return privateResponse(response);
    } catch {
      return errorResponse(502, 'BACKEND_UNAVAILABLE', 'Não foi possível contactar o servidor. Tenta novamente.');
    }
  },
};
